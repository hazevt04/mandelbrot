...
int main(int argc, char *argv[])
{
   ...
   strcpy( outfilename, argv[4] );
	...
	float *buffer = createMandelbrotImage(width, height, -0.802, -0.177, 0.011, max_iterations);
	if (buffer == NULL) {
		return 1;
	}
	...
	printf("Saving PNG\n");
	int result = writeImage(outfilename, width, height, buffer, "This is my test image");
